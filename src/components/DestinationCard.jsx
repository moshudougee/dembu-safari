...
const DestinationCard = ({ title, description, image, location }) => {
    let formatedTitle, formatedDesc = ''
    if(title.length > 29) {
      formatedTitle = title.substring(0, 29) + '...'
    } else {
      formatedTitle = title
    }
  ...
  return (
        <div className="flex flex-col rounded-lg border shadow-sm w-full m-0 hover:text-success-1 cursor-pointer">
            <div className='flex flex-col space-y-1.5 p-4'>
               <span className='text-xl xl:text-2xl  font-semibold leading-none tracking-tight'>{formatedTitle}</span>
